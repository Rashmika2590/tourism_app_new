...
class _SearchPageState extends State<SearchPage>
  ...
  // Add state variables for lat/lng
  ...
  void _search() async {
    ...
    String? locationName = _cityController.text.trim();

    // If no location is selected from dropdown, try to get coordinates from text input
    if ((lat == null || lng == null) &&
        locationName != null &&
        locationName.isNotEmpty) {
      try {
        final coords =
            await LocationService.getCoordinatesFromLocationName(locationName);
        lat = coords.latitude;
        lng = coords.longitude;
      } catch (e) {
        print("Could not get coordinates from location name: $e");
        // Fallback to current location below
      }
    }

    // If no location is available yet, use the current device location
    if (lat == null || lng == null) {
      try {
        ...
        // Handle error if location cannot be fetched
    ...
    // Navigate to the property list page with coordinates
    Navigator.push(
      context,
      MaterialPageRoute(
  ...
  // Fetch coordinates from place ID
  Future<void> _getCoordinatesFromPlaceId(String placeId) async {
    try {
      final response = await http.get(
        Uri.parse(
          'https://maps.googleapis.com/maps/api/place/details/json?place_id=$placeId&key=YOUR_GOOGLE_API_KEY&fields=geometry',
        ),
      );
      ...
      if (response.statusCode == 200) {
        ...
        if (data['status'] == 'OK') {
          ...
          print(
            '📍 Coordinates: $_selectedLatitude, $_selectedLongitude',
          );
        } else {
          print('Error from Place Details API: ${data['status']}');
        }
      } else {
  ...
  Future<List<Map<String, String>>> _getSuggestions(String query) async {
    try {
      final response = await http.get(
        Uri.parse(
          'https://maps.googleapis.com/maps/api/place/autocomplete/json?input=$query&key=YOUR_GOOGLE_API_KEY&types=(cities)',
        ),
      );
                ...
                child: TypeAheadField<Map<String, String>>(
                  ...
                  onSuggestionSelected: (suggestion) {
                    ...
                    if (placeId != null && description != null) {
                      // Set the text field to the selected description
                      _cityController.text = description;

                      // Fetch coordinates from the place ID
                      _getCoordinatesFromPlaceId(placeId);

                      // Debug print
                      print('🏙️ Selected: $description');
