...
class FavouriteApiService {
  ...
  // ====== ADD FAVOURITE ======
  static Future<Favourite> addFavourite(int hotelId) async {
    return await _makeAuthenticatedRequest<Favourite>(
      requestFunction: (token) async {
        final uri = Uri.parse("$baseUrl/favourite/");
        final body = jsonEncode({"hotel_id": hotelId});
        ...
        if (response.statusCode == 200 || response.statusCode == 201) {
          ...
          return Favourite.fromJson(responseData);
        } else {
          throw Exception("Failed to add favourite: ${response.body}");
        }
      },
  ...
  // ====== REMOVE FAVOURITE ======
  static Future<bool> removeFavourite(int hotelId) async {
    return await _makeAuthenticatedRequest<bool>(
      requestFunction: (token) async {
        final uri = Uri.parse("$baseUrl/favourite/$hotelId");
  ...
  // ====== GET USER FAVOURITES ======
  static Future<List<Hotel>> getUserFavourites() async {
    return await _makeAuthenticatedRequest<List<Hotel>>(
      requestFunction: (token) async {
        final uri = Uri.parse("$baseUrl/favourite/user");
        print("Fetching user favourites");
